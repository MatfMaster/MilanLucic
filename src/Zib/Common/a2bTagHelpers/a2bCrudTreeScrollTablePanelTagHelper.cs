...
namespace Zib.Common.A2bTagHelpers
{
    ...
    [HtmlTargetElement("a2b-crud-tree-scrolltable-panel")]
    public class A2bCrudTreeScrollTablePanelFieldTagHelper : TagHelper
    {
        ...
        [HtmlAttributeName("tree-column")]
        public string TreeColumn { get; set; }
        ...
        public override void Process(TagHelperContext context, TagHelperOutput output)
        {
            ...
            if (TreeColumn == null) TreeColumn = "naziv";
            ...
            sb.AppendLine($"            </div>");
            sb.AppendLine("            <h4><strong>{{ title }}</strong></h4>");
            sb.AppendLine($"        </div>");
            sb.AppendLine($"        <div class=\"body\">");
            sb.AppendLine($"            <tree-scrollable-table [filter]=\"{Filter}\"");
            sb.AppendLine($"                       [columns]=\"{Columns}\"");
            sb.AppendLine($"                       (rowChanged)=\"{OnRowchanged}($event)\"");
            sb.AppendLine($"                       [disabled]=\"{Disabled}()\"");
            sb.AppendLine($"                       treeColumn=\"{TreeColumn}\">");
            sb.AppendLine($"            </tree-scrollable-table>");
            sb.AppendLine($"        </div>");
