        ...
        <li *ngFor="let o of options" role="menuitem">
            <div class="ui-select-choices-row"
                ...
                <a href="javascript:void(0)" class="dropdown-item">            
                    <div [style.padding-left]="(o.nivo * 20) + 'px'" [innerHtml]="o.text | highlight:inputValue"></div>
                </a>
            </div>
        </li>
    </ul>
...
@Component({
  ...
  styles: [styles],
  template: `
      <div *ngIf="multiple === false" tabindex="0" 
          ...
          <div class="ui-select-match" *ngIf="!inputMode">
              <span tabindex="-1"
                  ...
                  <span *ngIf="active.length > 0" class="ui-select-match-text pull-left"
                      ...
                      [innerHTML]="active[0].text">
                  </span>

                  <i class="dropdown-toggle pull-right"></i>
                  <i class="caret pull-right"></i>

                  <a *ngIf="allowClear && active.length>0" style="margin-right: 10px; padding: 0;" (click)="remove(activeOption)" class="close pull-right">
                      &times;
                  </a>
              </span>
          </div>
      ...
      <div *ngIf="multiple === true" tabindex="0" 
        (keyup)="mainClick($event)"
        (focus)="focusToInput('')"
                ...
                <span *ngFor="let a of active">
                    <span class="ui-select-match-item btn btn-default btn-secondary btn-sm"
                        ...
                        [ngClass]="{'btn-default': true}">                    
                          <a class="close" style="margin-left: 10px; padding: 0;" (click)="remove(a)">&times;</a>
                        <span>{{a.text}}</span>
                    </span>
                </span>
            </span>
  ...
  @Input() public set items(value: Array<any>) {
    ...
    if (!value) {
      ...
      this._items = value.filter((item: any) => {
        // Ako je vrednost id = 0 (Root kod hijerarhije) bice preskocen
        //if ((typeof item === 'string' && item) || (typeof item === 'object' && item && item[this.idField] && item[this.idField])) {
        if ((typeof item === 'string' && item) || (typeof item === 'object' && item )) {
          return item;
        }
      }, this);
      ...
      //     console.log('string: ', this._items[i]);
      //     this.itemObjects.push(this._items[i]);
      //   } else {
      //     console.log('object: ', this._items[i]);
      //     this.itemObjects.push(new SelectItem({ id: this._items[i][this.idField], text: this._items[i][this.textField].toString(), nivo: this._items[i][this.nivo] }));
      //   }
      //   debugger;
      // }
      this.itemObjects = this._items.map((item: any) => { return (
          typeof item === 'string' ? 
          new SelectItem(item) : 
//          new SelectItem({ id: item[this.idField], text: item[this.textField].toString(), nivo: item[this.nivo] ? item[this.nivo] - 1 : 0 }))});
          new SelectItem({ id: item[this.idField], text: item[this.textField].toString(), nivo: item[this.nivo] }))});
    }
  }
  ...
  public set active(selectedItems: any) {
    ...
    if (typeof selectedItems === "number" && this._items){
      let selected = this._items.find((item: any) => { return item[this.idField] === selectedItems; })
      if (selected){
        let data =  { id: selected[this.idField], text: selected[this.textField] };
        this._active = [new SelectItem(data)];
      }
    } else if (Array.isArray(selectedItems) && this._items){
          for(let i = 0; i < selectedItems.length; i++){
              let selected = this._items.find((item: any) => { return item[this.idField] === selectedItems[i]; });
              if (selected){
                 let data =  { id: selected[this.idField], text: selected[this.textField] };
                 this._active.push(new SelectItem(data)); 
              }
          }
  ...
  // @Input()  public set active(selectedItems: Array<any>) {
  ...
  //     this._active = selectedItems.map((item: any) => {
  //       let data = areItemsStrings ? item : { id: item[this.idField], text: item[this.textField] };
  //       return new SelectItem(data);
  //     });
  //   }
  // }
  ...
  // Za ControlValueAccessor interface
  ...
  public remove(item: SelectItem): void {
    ...
    if (this.multiple === true && this.active) {
      ...
      for (let i = 0; i < this.active.length; i++){
        selektovani.push(this.active[i].id);
      }
      this._onChangeCallback(selektovani);
    }
  ...
  protected isActive(value: SelectItem): boolean {
    return this.active.find((item) => { return item.id === value.id; }) !== undefined;
  ...
  private selectMatch(value: SelectItem, e: Event = void 0): void {
    ...
    if (this.multiple === true) {
      ...
      this.element.nativeElement.querySelector('.ui-select-container').focus();
       this._onChangeCallback(this.active[0].id);
    }
  }
